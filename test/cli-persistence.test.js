...
test('watch dashboard hard reset clears all agents and history', async () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'poke-agents-persist-'));
  const watchPaths = getPersistencePaths('watch', tempRoot);
