...
class AgentState extends EventEmitter {
  ...
  recordOwnedPokemonDiscovery(speciesId, context = {}) {
    ...
    const source = context.source === 'evolution'
      ? 'evolution'
      : (context.source === 'owned' ? 'owned' : 'recruit');
    ...
    this.firstDiscoveryByPokemon[pokemonId] = {
      agentId: sourceAgent ? sourceAgent.agentId : (ownedPokemon && ownedPokemon.sourceAgentId) || null,
      agentName: source === 'evolution'
        ? 'Evolution'
        : (source === 'owned' ? 'Owned Pokemon' : 'Recruit'),
      provider: source,
      projectId: sourceAgent
        ? sourceAgent.projectId
        : (ownedPokemon && (ownedPokemon.assignedProjectId || ownedPokemon.sourceProjectId)) || null,
      sessionId: sourceAgent ? sourceAgent.sessionId : (ownedPokemon && ownedPokemon.sourceSessionId) || null,
  ...
  adoptOwnedPokemon(options = {}) {
    ...
    const pokedexChanged = this.recordOwnedPokemonDiscovery(speciesId, {
      source: 'recruit',
      sourceAgent,
      now
    });
  ...
  evolveOwnedPokemon(id, options = {}) {
    ...
    pokemon.updatedAt = Date.now();
    const pokedexChanged = this.recordOwnedPokemonDiscovery(pokemon.speciesId, {
      source: 'evolution',
      ownedPokemon: pokemon,
      now: pokemon.updatedAt
    });
    this.lastUpdate = pokemon.updatedAt;
    if (pokedexChanged) {
      this.emit('pokedex', this.pokedexSnapshot());
